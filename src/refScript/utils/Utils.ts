...
class Utils {
    ...
    static isShowLog:boolean = false;
    ...
    public static removeComponentTween(parent) : void {
    ...
    public static safeRemove(obj : any) : void {
    ...
    public static playMusic(resname:string, looptime : number = -1) {
     ...
     */
    public static numberRoation(label:any,currenteNumber:any,addNumber:any):void{
     ...
     * @param {obj 灰化对象}
     */
    public static ColorGrey(obj:any,isShow:boolean = true,isTouch:boolean = true):void{
        var colorMatrix = [0.3,0.6,0,0,0,0.3,0.6,0,0,0,0.3,0.6,0,0,0,0,0,0,1,0];
        var colorFlilter = new egret.ColorMatrixFilter(colorMatrix);
        if(isTouch)obj["enabled"] = !isShow;
        if(isShow){
     ...
     */
    public static soundArray:any = [];
    public static soundPlay(res:string,isBGM:boolean = false,loops:number = 1,volume:number = 1):egret.SoundChannel{
    ...
    public static getSoundObject():any[]{
        return Utils.soundArray;
    }

    public static showLog(message?: any, ...optionalParams: any[]):void{
        if(DEBUG){
    ...
    // 1代表麻将 2代表碰杠胡等 
    public static playMahjEffect(source : number = 1,sex : number = 1, name : string = '', type : number = 0, value : number = 0) : void {
    ...
    /**1,000,000,000*/
    public static numberFormat2(num:number,isUseLetter:boolean = false):string
    {
        if(isNaN(num)) return "";
     ...
     * @param isQ 是否缩写千
     */
    public static numberFormat(data:any,subIndex:number = 0,isUseLetter:boolean = false,isQ:boolean = false):string{
        var data_str:string=data+"";
        if(subIndex==0){
             var numberText = parseInt(data+"");
        }
        else if(subIndex>0){
            var dian_index:number=-1;
     ...
     * @param {subIndex 小数位数,默认不保留小数位}
     */
    public static numberFormat3(data:any,subIndex:number = 0):number{
        var data_str:string=data+"";
        var numberData:number=0.00;
        if(subIndex==0){
            numberData = parseInt(data+"");
        }
        else if(subIndex>0){
            var numberData=parseFloat(data);
            var dian_index:number=data_str.length;
            for(var i=0;i<data_str.length;i++){
                if(data_str[i]=="."){
                    dian_index=i;
                }
            ...
            // else{
            //     numberData = parseFloat(data_str.substring(0));                                       
            // }
        }        
        return numberData;
    }

    public static numberFormat4(data:any,subIndex:number = 0):string{
        var string = parseFloat(data).toFixed(subIndex);
        return string;
    }


    public static loadSoundFile(resname, cb : Function, thisobj : any) : void {
    ...
    public static removeDefaultResFile(resfile : string) : void {
    ...
    public static playEffect(resname:string, looptime:number = 1):egret.SoundChannel {
    ...
    //重新设置按钮锚点
    public static resetAnchor(obj: eui.Button)
    {
    ...
    //  放大缩小按钮
    public static scaleButton(obj : eui.Button)
    {
    ...
    public static addBtnEvent(btn : any, cb : Function, that : any, obj : any, type : number = 1, sound : string = '', delay : number = 0, preventall : boolean = false) : void {
        if (delay == 0) {
            delay = 200;
        }
    ...
    // android 内存一G的都不释放
    // 移动浏览器都释放
    public static checkReleaseLobbyRes() : void {
    ...
    public static loadMovieClipFile(resname : string, cb : Function, thisobj : any) : void {
    ...
    public static destroyMovieClipFile(resname : string) : void {
        RES.destroyRes(resname + "_json");
        RES.destroyRes(resname + "_png");
    }

    public static createMovieClipByName(resname : string, clipname : string) {
        ...
        var mcDataSet:any = mcf.mcDataSet;
        var clip : egret.MovieClip = new egret.MovieClip(mcf.generateMovieClipData(clipname));
    ...
    // 杠    +1番
    // 从大的开始计算
    public static caculateFan(pais : any) : number {
        // 获取花色数量
        // 万 1-9 条 17-25 //筒 33-41
        var getHuase = function(pais) : number {
            var count = [0, 0, 0]
    ...
    // 获取牌里面有哪些对子
    public static getDuizi(pais : any) : any {
        var ret = [];
        var temp = {};
        for (var i = 0; i < pais.length; ++i) {
    ...
    public static checkHuPrompt(pais : any) : any {
    ...
    // 查看是否副牌
    public static CheckHuPai(pais : any) : boolean {

        pais.sort((a, b) => {
            return a - b;
        });
        // 默认情况是排序了的
        var checkPai = function(leftpai) : boolean {
            // 这时不用考虑碰牌等情况 因为碰、杠不在牌序列里面
            // 判断是否满足n（ABC）的规则()

            var getThree = function(pp, sp:number, start:number) : any {
                var ret = {same: [], delta: []};
    ...
    // 5      4     3    2      1
    // AAA > ABC > AAB > ABD > ACE
    public static getPaiWeight(pais) : number {
        pais.sort((a, b) => {
            return a - b;
        });
    ...
    public static cutStrWithDot(str, len){
        var char_length = 0;
    ...
    public static convertToTextFlow(textStr : string) : any {
        var size = 24;
        var textflow = [];
    ...
    public static checkMobile(phone) : boolean {
    ...
    // proto文件解密
    public static decryptProtoFile(data : any, force : boolean = false) : string {
        if (data) {
    ...
    private static _configIsLoadDic:any = {};
    ...
    public static loadRemoteSource(url : string, cb : Function) : void {
    ...
    public static  GetQueryString(name : string, href : string) {
    ...
    public static loadHeadImg(strfaceid : string, headimg : any) : void {
        ...
        if ('' + faceid == strfaceid) {
            ...
            Utils.loadRemoteSource(GlobalPara.STATIC_HEAD_URL + "/image/" + strfaceid + ".png?v=" + Utils.rand(100000, 1000000), (texture) => {
                // if(headimg.texture)
                // {
    ...
    // remove 表示是否删除或者关闭
    public static sliderView(view : any, direction : number, remove : boolean, cb : Function = null) : void {
    ...
    public static getHeadRes(strfaceid : string):string
    {
    ...
    public static playUIAnimation(target:egret.tween.TweenGroup,isLoop:boolean) : void {
        if(isLoop)
        {
    ...
    /**字节数*/
    public static getStrByteCount(str:string):number{
     ...
     * 循环缓动
     */
    public static loopTween(target: any, props1?: any, duration1?: number, props2?: any, duration2?: number,waitTime:number = 0,ease1:any = null,ease2:any = null):void
    {
        //奔驰宝马
     ...
     * 带权值随机 ------ 返回权重对应的下标
     */
    public static randomByWeight(weightArr:Array<number>):number
    {   
        var  len:number = weightArr.length;
        if(len <2){
            return -1;
        }
        var  sum :number = 0;
       ...
       //随机出1 --sum 的一个随机数
       var hbIndex:number = Math.floor(Math.random() * sum + 1);
       var  index:number = 0;
       for(var  idx:number =0; idx<len; idx++){
          hbIndex -= weightArr[idx];
          if(hbIndex <= 0){ // 找到对应的权重下标
              index =  idx;
              break;
          }
       }
    ...
    // 获取外网ip地址
    public static getRemoteIp() : void {

        if (GlobalPara.isNative && GlobalPara.myip == "unknown") {

            var request = new NetProxy();
            request.request('http://ip.taobao.com/service/getIpInfo.php?ip=myip', null, (data, error) => {
    ...
    public static checkDateIsSame(date1:number,date2:number):boolean
    {
        if(date1 <= 0 || date2 <= 0) return false;
        let num1:number =  Math.floor(date1 / (24 * 60 * 60));
        let num2:number = Math.floor(date2 / (24 * 60 * 60) /1000);
